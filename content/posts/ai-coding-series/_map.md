...
- **별도 Astro 인덱스 페이지 없음.** `00-overview.md`(들어가며)가 개요·랜딩 역할을 겸한다.
- **두 시리즈 모두 동일한 방식**: `llm-agents-series`도 `00-overview.md`(들어가며)로 통일(Astro 인덱스 페이지는 쓰지 않음). 홈은 `series_order` 없는 개요 글을 목록 맨 앞에 자동 노출.
