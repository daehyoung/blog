...
다른 모델은 **다른 분포**입니다. 학습 데이터도, 정렬 방식도, 몸에 밴 습관도 다릅니다. 그래서 원저자 모델이 구조적으로 못 보는 것을 봅니다 — 명부가 다른 유권자를 부르는 셈입니다.

다만 **완전히 다른 명부는 아닙니다.** 상용 모델들은 학습 데이터가 크게 겹치고 정렬 방식도 비슷해서 **평균의 상당 부분을 공유합니다.** 독립 표본이라기보다 *같은 학교 출신 심사위원 둘*에 가깝습니다(같은 회사 모델끼리면 더 그렇습니다). 그래서 서열은 유지됩니다 — **자기 채점 < 새 세션 < 다른 모델** — 하되, **다른 모델도 사람 게이트를 대체하지는 못합니다.** 새 세션을 여는 것보다 한 단계 강한 조치이고, 층 5(선택압)가 가장 세게 작동하는 지점입니다.
...
단, 금지에는 알려진 약점이 하나 있습니다 — **금지어를 언급하는 것 자체가 그 개념을 불러오는** 역효과입니다("분홍 코끼리를 생각하지 마"). 그래서 금지 항목에는 되도록 **대체 행동을 함께 붙입니다.** 위 목록의 `→ 멈추고 물을 것`, `→ 자리 표시자로 두고 5단계로`가 그 장치입니다. 흉내 낼 때 **금지만 나열하면 오히려 역효과**가 날 수 있습니다.
...
**둘째, "이미 기각됨" 한 줄.** 회의 글에서 늘 나오는 처방들이 있습니다 — 아젠다를 명확히, **타임박싱**(회의마다 시간 상한을 두는 것), 참석자 최소화. A에는 그게 그대로 들어 있고, B에는 없습니다.

없어진 이유가 두 겹인데, 뒤쪽이 더 중요합니다.

- **금지 목록**이 그 항목들을 직접 막습니다. 흔한 답이 뭔지 알고 그 지점을 정확히 차단한 것입니다.
- **맥락 한 줄**은 더 넓게 작동합니다. *"작년에 30분 제한을 도입했으나 회의 수가 늘어 총시간은 그대로였다."* 이 한 줄은 열거된 항목 몇 개가 아니라 **"회의 운영 규칙을 손보는" 처방 전체를 무력화**하고, 동시에 글의 논지("손댈 곳은 회의가 아니라 논의량이다")에 근거를 줍니다. B가 그 실패를 1절에서 자기 데이터로 쓰는 이유가 이것입니다.

금지 목록은 **내가 미리 아는 함정**만 막습니다. 실패 기록은 **내가 미처 열거하지 못한 것까지** 막습니다. 프롬프트 기교가 아니라 **조직이 가진 실패 기록**이 한 일이고, 4편에서 말할 자산이 실제로 작동하는 장면이 이겁니다.
