...
- **신규 독립 시리즈 「모델에게 SW 엔지니어링 가르치기」**(AI 코딩 실전의 *심화 자매 시리즈*) — `content/posts/teaching-swe-to-models/`, URL `/blog/teaching-swe-to-models/...`:
  ...
  - **발행: 7편 동시(단일 드롭) `date: 2026-07-25`.** 이유 — 앞뒤 교차링크 밀도가 높아 drip 시 미공개 뒷편 링크가 404(글 페이지가 날짜 게이트 탐). 동시 발행이라 로드맵·"다음 편"을 실제 링크로 처리.
  - **링크 검증 완료**: 임시로 오늘 날짜로 내려 7편 페이지 생성 → 내부 링크 18개 전부 dist 존재(MISS 0) → 07-25 복원. 클린 빌드 통과(69p, 게이트 아웃 정상). **아직 미커밋/미푸시.**
...
0. **[대기] 신규 시리즈 「모델에게 SW 엔지니어링 가르치기」 7편(개요+5+에필로그) 커밋/푸시** — 작성·빌드·링크검증 끝. 기밀문서(test_strategy)는 제거해 **공개 콘텐츠에 기밀 없음**. 확인 후 커밋+SSH 푸시(샌드박스 해제) → 07-25 cron 동시 공개.
