...
- ⚠️ **강조가 닫히지 않는 함정 (한국어 글의 상시 위험)**: `**명세(유스케이스)**를` 처럼 **강조 안쪽이 구두점으로 끝나고 바로 조사가 붙으면 볼드가 닫히지 않고 별표가 그대로 노출된다.** CommonMark의 right-flanking 규칙(닫는 구분자 앞이 구두점이면 뒤가 공백·구두점이어야 함) 때문이고, 한국어는 괄호·인용부호 뒤에 조사가 바로 붙어 늘 걸린다. **조사 자체는 문제가 아니다** — `**직교**한다`는 정상이다.
  - **빌드가 못 잡는다.** 문법 오류가 아니라 그냥 리터럴 텍스트다. 실제로 34개 파일 90여 곳이 별표를 노출한 채 배포돼 있었다.
  - 새 글·수정 후 `python3 scripts/check-emphasis.py` 로 검사한다(강조 + 물결표 둘 다 본다).
  - 고치는 순서: ① 감싼 기호를 밖으로(`**"A"**이다` → `"**A**"이다`) ② 뒤 괄호를 밖으로(`**전문가(expert)**로` → `**전문가**(expert)로`) ③ 뜻이 바뀌면 `<strong>`·`<em>` 태그. **띄어쓰기로 우회하지 말 것**(맞춤법이 틀어진다).
  - 모델 출력을 그대로 인용한 자리는 **고치지 않는다** — 기록이다(예: `river-crossing-experiment`).
- ⚠️ **물결표는 반드시 `\~` 로 이스케이프**: GFM은 **물결표 하나로도 취소선**을 만든다. `2~3주 → 1~2일` 이 `2<del>3주 → 1</del>2일` 로 깨진 적이 있다(에필로그). 범위(`2\~3주`)·근사(`\~10×`) 표기에 특히 주의.
